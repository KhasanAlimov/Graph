...
// URL JSON файла
const URL = 'https://raw.githubusercontent.com/KhasanAlimov/JSON-files/master/Company-list.json';
...
fetch(URL)
    ...
    .then(data => {
        ...
        let companyNameIndex = 0; // Индекс перебираемого массива 
        ...
        let companiesButton = document.querySelectorAll('#company-switcher');
        companiesButton[0].classList.add('active-graph');

        // Добавление активного класса
        companiesButton.forEach(item => {
          item.addEventListener('click', function () {
          companiesButton.forEach(item => {
            item.classList.remove('active-graph')
          })
          this.classList.add('active-graph')
          })
        })
        ...
        let path; // Переменная для графика
        let dots; // Переменная для горячих точек
        // Кнопки показа горячих точек
        let showDots = d3.select('#svg_container')
                    .append('button')
                    .text('Показать горячие точки')
                    .attr('id', 'show-dots');
        ...
        let graph = () => {
            ...
            path = svg.append("path")
                ...
                // При клике на кнопку показать горячие точки
                showDots.on('click', () => {
                  if (dots) {
                    dots.remove();
